...
**Current checkpoint:** 17H (Dashboard + final integration) implemented and validated; settings next.
...
**Dev login:** `npm run seed:dev --workspace=server` creates org `abc` + full Admin (`18dakshsuri@gmail.com` / same as password); prints the org ID for the login page. Upsert-only, refuses `NODE_ENV=production`. Dev-only credentials, never seed prod.
